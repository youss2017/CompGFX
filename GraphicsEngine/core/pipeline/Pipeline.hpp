...
struct PipelineVertexInputDescription
{
    ...
    void GRAPHICS_API AddInputElement(const std::string &name, uint32_t location, uint32_t binding_id, uint32_t vector_size, bool isfloatingpoint, bool isunsigned, bool per_instance, uint32_t divisor_rate = 1)
    {
    ...
    uint32_t GRAPHICS_API GetDivisorRate(uint32_t bindingID)
    {
        for (auto &e : m_input_elements)
