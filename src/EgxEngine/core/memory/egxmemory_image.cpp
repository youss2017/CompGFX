    ...
    VkImageLayout InitalLayout)
{
    ...
    auto result = ref<Image>(new Image(width, height, depth, mipcount, desc.m_arrayLayers,
                                       image->m_image, desc.m_usage, CoreInterface->MemoryContext, image, CoreInterface, format, aspect, layout, InitalLayout));
    ...
    uint32_t StrideSizeInBytes)
{
    ...
    ref<Buffer> stage = Buffer::FactoryCreate(_coreinterface, Height * StrideSizeInBytes, memorylayout::stream, buffertype::buffertype_onlytransfer, false);
    stage->Write(Data);
    stage->Flush();
    ...
    vkCmdCopyBufferToImage(cmd.Cmd, stage->GetBuffer(), Img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
{
    return FactoryCreate(_coreinterface, _memorylayout, ImageAspect, Width, Height, Depth, Format, Mipcount, Arraylevels, _imageusage, _initial_layout);
}
