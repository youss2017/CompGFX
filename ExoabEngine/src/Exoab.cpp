...
static ShaderTypes::ObjectData* s_ObjectDataPtr[gFrameOverlapCount];
static ShaderTypes::SceneData* s_SceneData[gFrameOverlapCount];
static ShaderTypes::DrawData* s_DrawDataPtr[gFrameOverlapCount];
...
static ShaderTypes::TerrainTransform* s_TerrainTransforms[gFrameOverlapCount];
...
bool Exoab_Initalize(ConfigurationSettings config)
{
    ...
    terrain = Terrain_Create(s_Context, s_Sampler0, 40, 2, 40, 2, s_Tex0, {});
    ...
    ShaderBinding_CalculatePoolSizes(gFrameOverlapCount, poolSizes, set1Bindings);
    s_SetPool = vk::Gfx_CreateDescriptorPool(s_Context, 2 * gFrameOverlapCount, poolSizes);
    ...
    for (unsigned int j = 0; j < gFrameOverlapCount; j++)
    {
    ...
    for (unsigned int j = 0; j < gFrameOverlapCount; j++) {
        ...
        s_TerrainTransforms[j] = (ShaderTypes::TerrainTransform*)Buffer2_Map(terrain->m_Set0->m_bindings[1].m_buffer[j]);
    }
...
void BuildCommandBuffers(FrameData& frame);
...
void Exoab_Render(double dTimeStart, double dElapsedTime, double FrameRate)
{
    ...
    FrameData& frame = Graphics3D_GetFrame(s_Gfx);
    VkDevice device = s_Context->defaultDevice;

    vkcheck(vkWaitForFences(device, 1, &frame.m_RenderFence, true, UINT64_MAX));
    vkcheck(vkResetFences(device, 1, &frame.m_RenderFence));
    vkcheck(vkResetCommandBuffer(frame.m_cmd, 0));

    if (s_Swapchain.PrepareNextFrame(UINT64_MAX, frame.m_PresentSemaphore, nullptr, &frame.m_FrameIndex) != VK_SUCCESS)
        return;
    const uint32_t& FrameIndex = frame.m_FrameIndex;
    ...
    glm::vec3 scale = glm::vec3(45., 45., 45.);
    s_TerrainTransforms[FrameIndex]->u_Model = glm::scale(glm::mat4(1.0), scale);
    s_TerrainTransforms[FrameIndex]->u_NormalModel = glm::inverse(glm::transpose(glm::scale(glm::mat4(1.0), scale)));
    s_TerrainTransforms[FrameIndex]->u_Projection = (glm::perspective(glm::radians(90.0f), (float)s_Window->m_width / (float)s_Window->m_height, 0.1f, 2000.f));
    s_TerrainTransforms[FrameIndex]->u_View = s_Camera.GetViewMatrix();
    Buffer2_Flush(terrain->m_Set0->m_bindings[1].m_buffer[FrameIndex], 0, sizeof(ShaderTypes::TerrainTransform));
    ...
    submitInfo.pWaitSemaphores = &frame.m_PresentSemaphore;
    submitInfo.pWaitDstStageMask = flags;
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &frame.m_cmd;
    submitInfo.signalSemaphoreCount = 1;
    submitInfo.pSignalSemaphores = &frame.m_RenderSemaphore;
    vkQueueSubmit(s_Context->defaultQueue, 1, &submitInfo, frame.m_RenderFence);

    UI::FrameRate = FrameRate;
    UI::RenderUI();
    s_Swapchain.Present(s_Material0.m_framebuffer->m_textures[UI::ShowDepthBuffer]->m_vk_images_per_frame[FrameIndex], s_Material0.m_framebuffer->m_textures[UI::ShowDepthBuffer]->m_vk_views_per_frame[FrameIndex], VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1, &frame.m_RenderSemaphore, false);
    Graphics3D_NextFrame(s_Gfx);
}
...
void BuildCommandBuffers(FrameData& frame)
{
    ...
    const VkCommandBuffer& cmd = frame.m_cmd;
    ...
    BeginInfo.framebuffer = s_Material0.m_framebuffer->m_framebuffer->m_framebuffers[frame.m_FrameIndex];
    ...
    VkDescriptorSet sets[2] = { s_Set0->m_set[frame.m_FrameIndex], s_Set1->m_set[frame.m_FrameIndex] };
    ...
    vkCmdDrawIndexedIndirectCount(cmd, s_Set0->m_bindings[3].m_ssbo[frame.m_FrameIndex]->m_vk_buffer->m_buffer, 0, s_DrawCountBuffer->m_vk_buffer->m_buffer, 0, s_Geomtry.size(), sizeof(ShaderTypes::DrawData));
    
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, (VkPipeline)tpipeline->m_pipeline);
    sets[0] = terrain->m_Set0->m_set[frame.m_FrameIndex];
    sets[1] = terrain->m_Set1->m_set[frame.m_FrameIndex];
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, tlayout->m_layout, 0, 2, sets, 0, nullptr);
    vkCmdBindIndexBuffer(cmd, terrain->m_indices_buffer->m_vk_buffer->m_buffer, 0, VK_INDEX_TYPE_UINT32);
    glm::vec3 LightDirection = glm::vec3(0, 1, 0);
    vkCmdPushConstants(cmd, terrain->m_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(glm::vec3), &LightDirection);
...
Tristate Exoab_Update(double dTimeStart, double dElapsedTime)
{
    ...
    if (UI::StateChanged)
    {
        ...
        tpipeline = PipelineState_Create(s_Context, spec2, s_Material0.m_state_managment, s_Material0.m_pipeline_layout->input_description, tlayout->m_layout, tshaders->m_vertex_shader, tshaders->m_fragment_shader);
    }
