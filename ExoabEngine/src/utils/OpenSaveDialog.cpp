...
std::string Utils::CreateDialogFilter(std::vector<std::pair<std::string, std::string>> items) {
	std::string filter = "";
...
std::string Utils::SaveAsDialog(const std::string& filter, int& OutChoosenFilterIndex) {
	char szFile[256]{0};
	OPENFILENAMEA open{};
	open.lStructSize = sizeof(OPENFILENAMEA);
	open.hwndOwner = glfwGetWin32Window(Global::Window->GetWindow());
