...
IBuffer2 Buffer2_Create(GraphicsContext _context, BufferType type, size_t size, BufferMemoryType memoryType)
{
	...
	desc.m_properties = memoryType == BufferMemoryType::STATIC ? DEVICE_MEMORY_PROPERTY::GPU_ONLY : ((memoryType == BufferMemoryType::STREAM) ? DEVICE_MEMORY_PROPERTY::CPU_TO_GPU : DEVICE_MEMORY_PROPERTY::CPU_ONLY);
	desc.m_size = size;
	desc.m_usage = memoryType == BufferMemoryType::STATIC ? VK_BUFFER_USAGE_TRANSFER_DST_BIT : 0;
	switch (type)
	{
		...
		case BufferType::IndirectBuffer: 
			...
			break;
		case BufferType::INTE_TRANSFER_SRC: 
			desc.m_usage |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
			break;
		case BufferType::INTE_TRANSFER_DST: 
...
void Buffer2_UploadData(IBuffer2 buffer, char8_t* pData, size_t offset, size_t size)
{
	...
	IBuffer2 intermediate = Buffer2_Create(buffer->m_context, BufferType::INTE_TRANSFER_SRC, size, BufferMemoryType::DYNAMIC);
