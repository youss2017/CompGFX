...
ITexture2 Application::GenerateMinimap(Terrain* terrain, std::vector<ITexture2> terrainTextures, ITexture2 normalMap)
{
	...
	fbo.m_width = 256;
	fbo.m_height = 256;
	FramebufferAttachment colorAttachment = FramebufferAttachment::Create(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, fbo.m_width, fbo.m_height, VK_FORMAT_R8G8B8A8_UNORM, { 0.0 });
	FramebufferAttachment depthAttachment = FramebufferAttachment::Create(VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, fbo.m_width, fbo.m_height, VK_FORMAT_D32_SFLOAT, { 1.0 });
	...
	VkDescriptorPool pool = vk::Gfx_CreateDescriptorPool(3, poolSizes);
	...
	auto state = PipelineState_Create(spec, input, fbo, layout, &vertex, &fragment);
