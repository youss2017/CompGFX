...
Application::GeometryPass::GeometryPass(uint32_t lightCount, ShaderTypes::Light* lights, IBuffer2 verticesSSBO, IBuffer2 indicesSSBO, Terrain* terrain, int width, int height, FrustumCullPass* cullPass, Camera* camera, Camera* lockedCamera, ecs::EntityController* ecs, ITexture2 shadowMap) : Pass(Global::Context->defaultDevice, true), mCamera(camera), mLockedCamera(lockedCamera), mECS(ecs), mIndicsSSBO(indicesSSBO), mCullPass(cullPass), mLightCount(lightCount), mLights(lights) {
	...
	FramebufferAttachment colorAttachment = FramebufferAttachment::Create( VK_IMAGE_USAGE_STORAGE_BIT, width, height, VK_FORMAT_B10G11R11_UFLOAT_PACK32, { 0.5, 0.5, 0.6, 0 });
	FramebufferAttachment depthAttachment = FramebufferAttachment::Create( 0, width, height, VK_FORMAT_D32_SFLOAT, { 1.0f });
	...
	auto transitionCmd = vk::Gfx_CreateSingleUseCmdBuffer();
	...
	mT0 = terrain;
	mShadowSampler = vk::Gfx_CreateSampler(
		VK_FILTER_LINEAR,
		VK_FILTER_LINEAR,
	...
	mWoodTex = Texture2_CreateFromFile("assets/textures/wood.png", true);
	mSandTex = Texture2_CreateFromFile("assets/textures/sand.jpg", true);
	mStatueTex = Texture2_CreateFromFile("assets/textures/statue.jpg", true);
	mNormalMap = Texture2_CreateFromFile("assets/textures/normal.jpg", true);
	mLightDirection = glm::vec4(0.0);
	mQuery = vk::Gfx_CreateQueryPool( VK_QUERY_TYPE_TIMESTAMP, gFrameOverlapCount * 2, 0);
	mInvocationQuery = vk::Gfx_CreateQueryPool( VK_QUERY_TYPE_PIPELINE_STATISTICS, gFrameOverlapCount, VK_QUERY_PIPELINE_STATISTIC_VERTEX_SHADER_INVOCATIONS_BIT | VK_QUERY_PIPELINE_STATISTIC_FRAGMENT_SHADER_INVOCATIONS_BIT);
	...
	mPool = vk::Gfx_CreateDescriptorPool( gFrameOverlapCount * 2 + (gFrameOverlapCount * 1), poolSizes);
	...
	mGeoState = PipelineState_Create(spec, input, mFBO, mGeoLayout, &geoVertex, &geoFragment);
	...
	mMapState = PipelineState_Create(mGeoState->m_spec, input, mFBO, mMapLayout, &mapVertex, &mapFragment);
...
void Application::GeometryPass::ReloadShaders() {
	...
	Shader mapFragment = Shader("assets/shaders/terrain/Terrain.frag");
	PipelineVertexInputDescription input;
	mGeoState = PipelineState_Create(specification, input, mFBO, mGeoLayout, &geoVertex, &geoFragment);
	mMapState = PipelineState_Create(specification, input, mFBO, mMapLayout, &mapVertex, &mapFragment);
	for (int i = 0; i < gFrameOverlapCount; i++) {
		RecordCommands(i);
	}
}
